...
$siblingAssemblyMinutesCorrespondence = AssemblyMinutesCorrespondence::search([
        ...
        ['owner_id', '=', $assemblyMinutesCorrespondence['owner_id']],
        ['document_id', '<>', null]
    ])
    ->read(['document_id'])
    ->first();

if($siblingAssemblyMinutesCorrespondence) {
    $document_id = $siblingAssemblyMinutesCorrespondence['document_id'];
}
...
if($document_id) {
    ...
    AssemblyMinutesCorrespondence::id($assemblyMinutesCorrespondence['id'])->update(['document_id' => $document_id]);
}
