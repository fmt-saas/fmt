...
class BankStatementLine extends Model {
        ...
        return [
            ...
            'transaction_type' => [
                ...
                'selection'         => [
                    ...
                    'direct_debit_fee',
                    'direct_debit_in',
                    'direct_debit_credit_provisional',
                    ...
                    'account_settlement',
                    'account_settlement_out',
                    'account_settlement_fee',
                    'account_settlement_in',
    ...
    public static function getActions() {
        return [
            ...
            'assert_funding' => [
                'description'   => 'Make sure a funding (non expeccted amount) exists for the Bank statement line.',
                'policies'      => [],
                'function'      => 'doAssertFunding'
            ],
    ...
    protected static function doAssertFunding($self) {
        $self->read([
                    'status',
                    'condo_id',
                    'communication',
        ...
        foreach($self as $id => $bankStatementLine) {
            ...
            try {
                ...
                if(abs($remaining_amount) >= 0.01) {
                    ...
                    if(!$funding) {
                        $logs[] = "ERR  - Unable to create/retrive bank statement line funding";
                    }
                    else {
                        $logs[] = "INFO - Using statement-line funding {$funding['id']}";
    ...
    private static function computeFundingCandidates($bank_statement_line_id): array {
        ...
        if($bankStatementLine['is_supplier']) {
            ...
            if(!count($fundings_ids)) {
                // 2) attempt to find a match with remaining amount only
                $fundings_ids = Funding::search([
                            ['condo_id', '=', $bankStatementLine['condo_id']],
                            ['accounting_account_id', '=', $bankStatementLine['accounting_account_id']],
                            ['status', '<>', 'balanced'],
            ...
            if(!count($fundings_ids)) {
                // 3) fallback to all non-balanced fundings
                $fundings_ids = Funding::search([
                            ['condo_id', '=', $bankStatementLine['condo_id']],
                            ['accounting_account_id', '=', $bankStatementLine['accounting_account_id']],
                            ['status', '<>', 'balanced'],
    ...
    private static function computeIsReconciled($id) {
        ...
        return ( abs(round($sum, 2) - round($bankStatementLine['amount'], 2)) < 0.01 );
    }
