...
[$params, $providers] = eQual::announce([
    'description'       => "Delete instance using the b2 API.",
    'params'            => [
        'id' =>  [
            'type'              => 'many2one',
        ...
        'delete_b2_instance' => [
            'type'              => 'boolean',
            'description'       => "Delete the b2 instance.",
            'default'           => false,
            'required'          => true
        ]
    ],
...
$instance = Instance::id($params['id'])
    ->read(['name', 'user_id', 'server_id' => ['b2_api_url', 'b2_api_password']])
    ->first();
...
if($params['delete_b2_instance']) {
    ...
    if($status < 200 || $status > 299) {
        throw new Exception('unable_to_delete_b2_instance', EQ_ERROR_UNKNOWN);
    }
}

if($instance['user_id']) {
    User::id($instance['user_id'])->delete(true);
}

Instance::id($instance['id'])->delete(true);
