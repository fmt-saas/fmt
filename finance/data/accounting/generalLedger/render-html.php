...
$data = eQual::run('get', 'finance_accounting_AccountingEntryLine_collect', [
        'entity'            => 'finance\accounting\AccountingEntryLine',
        'fields'            => ['condo_id.name','account_id.name','journal_id.name','accounting_entry_id.name','entry_date','description','debit','credit','status'],
...
usort($groups, function ($a, $b) {
    // Code nous garantit déjà le bon ordre comptable
    $cmp = strcmp($a['account_code'], $b['account_code']);
    if ($cmp !== 0) {
        return $cmp;
    }

    // Si même code (rare), on trie par label
