...
class AssemblyMinutesInstance extends \equal\orm\Model {
    ...
    protected static function onupdateDocumentId($self) {
        ...
        foreach($self as $id => $assemblyMinutesInstance) {
            self::id($id)->update(['has_document' => (bool) $assemblyMinutesInstance['document_id']]);
        }
    }
    ...
    protected static function canupdate($self, $values) {
        ...
        foreach($self as $id => $assemblyMinutesInstance) {
            if($assemblyMinutesInstance['is_sent']) {
