...
$assemblyInvitation = AssemblyInvitation::id($params['id'])
    ->read(['status', 'condo_id', 'ownership_id', 'communication_method', 'name'])
    ->first();
...
if($assemblyInvitation['communication_method'] !== 'email') {
    ...
    $data = eQual::run('get', 'realestate_governance_AssemblyInvitation_render-pdf', ['id' => $assemblyInvitation['id']]);
    ...
    AssemblyInvitation::id($assemblyInvitation['id'])->update(['document_id' => $document['id']]);
}
