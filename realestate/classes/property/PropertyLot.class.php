...
class PropertyLot extends \equal\orm\Model {
    ...
    public static function getLink() {
        return "/app/#/condo/:condo_id/property-lot/object.id";
    }
    ...
    public static function cancreate($self, $values): array {
        Quota::search([['code', '=', 'property.main_lots.count']])
            ->do('check-availability', $values);

        $parkingPropertyLotNature = PropertyLotNature::search(['code', '=', 'PARKING'])
                ->read(['id'])
                ->first();

        if($parkingPropertyLotNature && isset($values['nature_id']) && $values['nature_id'] === $parkingPropertyLotNature['id']) {
            Quota::search([['code', '=', 'property.parkings.count']])
                ->do('check-availability', $values);
        }
